...
`Acceso` http://sso-service-provider-nfeuat.nfedev.accelya.com
...
- server.port = 80 #Port in which the application will be run. 
...
- service-provider.hostname = The name of the host where the service is running at the moment.
...
Right now the certificate is already generated so is not necessary to generate anyone.
...
- Single sign on URL: http://sso-service-provider-nfeuat.nfedev.accelya.com:80/saml/SSO
- Service Provider Entity ID: http://sso-service-provider-nfeuat.nfedev.accelya.com:80/saml/metadata
