...
export class PageRowsHelper {
  ...
  findPage(page:number):IPageRows{
    return this.listPagesRows.find((pageRows:IPageRows)=> pageRows.page==page);
  }
  ...
  delPage(page:number){
     this.listPagesRows= this.listPagesRows.filter(((pageRows:IPageRows)=>pageRows.page!=page));
  }
  getAllData():any[]{
    let allData = [];
