...
describe('BasicInfoService', () => {
  ...
  const _AgentService = jasmine.createSpyObj<AgentService>('AgentService', ['getAgentWithCode', 'setBaseURL']);
  const _CompanyService = jasmine.createSpyObj<CompanyService>('CompanyService', ['getFromServerAirlineCountryAirlineCode', 'setBaseURL']);
  ...
  const _CurrencyService = jasmine.createSpyObj<CurrencyService>('CurrencyService', ['getCurrency', 'getCurrencyWithISO', 'setBaseURL']);
  ...
  _CurrencyService.setBaseURL.and.returnValue(Observable.of({}));
