...
describe("FilesDownloadComponent", () => {
  ...
  it("load data files on view init", async(() => {
    // const service = fixture.debugElement.injector.get(ListFilesService);
    fixture.detectChanges();
    //spyOn(service, 'listFilesData' ).and.callThrough();
    fixture.whenStable().then(() => {
      console.log("whennn stable");
    });
    console.log("ppppppppp inittttttt");
    fixture.detectChanges();
...
it('selection data files on table', async(() => {
  ...
  spyOn(component,'buttest');
  const btn = fixture.debugElement.query(By.css('#buttest'));
  btn.triggerEventHandler('click',null);
  fixture.detectChanges();
  expect(component.buttest).toHaveBeenCalled();
})); */
it('delete  from  selection data files ', (() => {
  const spyRemove = fileService.removeFile.and.returnValue(of({}));
 // spyOn(component,'refreshQuery');
  fixture.detectChanges();
  expect(component.tablePagination.table).toBeDefined();
  expect(component.tablePagination.table.getrows().length).toBe(3);
  component.tablePagination.table.selectedrowindex(1);
  ...
  //btn.nativeElement.triggerEventHandler('click',null);
  fixture.detectChanges();
...
it('page changes  ', (() => {
  const updateDataSpy = spyOn(component.tablePagination, 'updatePaginationData').and.callThrough();
  fixture.detectChanges();
