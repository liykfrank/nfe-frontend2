...
describe('FilesUploadComponent', () => {
  ...
  const ConfigurationServiceSpy = jasmine.createSpyObj('ConfigurationService', ['get']);
  ConfigurationServiceSpy.get.and.returnValue(Observable.of(expectedConfiguration));
  ...
  it('#isValidNumFiles should save an error message', () => {
    ...
    expect(component.msgs[0]).toEqual({ severity: 'error', summary: errMsg, detail: '' });
  });
  ...
  it('#onSelect should select 2 files', () => {
    ...
    component.ngOnInit();
    // Multiple upload
    component.myFileUpload.files = [file1, file2, file3, file4, file5, file6, file7];
    component.onSelect(event);

    expect(component.myFileUpload.files.length).toEqual(2);
    expect(component.myFileUpload.files[0]).toEqual({name: 'abc.xyz', size: 1024});
    expect(component.myFileUpload.files[1]).toEqual({name: 'ESec111111.xyz', size: 1024});
