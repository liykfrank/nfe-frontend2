...
function findExecutableLines(sourceFile) {
    ...
    walk(sourceFile, node => {
        if (EXECUTABLE_STATEMENT_KINDS.includes(node.kind)) {
            lines.add(sonarUtils_1.toSonarLine(navigation_1.lineAndCharacter(node.getStart(), sourceFile).line));
        }
    });
    return Array.from(lines).sort((a, b) => a - b);
}
...
function addLines(start, end, lines, sourceFile) {
    const firstLine = sonarUtils_1.toSonarLine(navigation_1.lineAndCharacter(start, sourceFile).line);
    const lastLine = sonarUtils_1.toSonarLine(navigation_1.lineAndCharacter(end, sourceFile).line);
