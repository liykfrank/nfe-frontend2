...
class Visitor extends sonarAnalysis_1.SonarRuleVisitor {
    ...
    countStatementsInTheSamePile(reference, statements) {
        ...
        let startOfPile = navigation_1.lineAndCharacter(reference.getStart(), file);
        let lastLineOfPile = startOfPile.line;
        for (const statement of statements) {
            const currentLine = navigation_1.lineAndCharacter(statement.getEnd(), file).line;
            const currentIndentation = navigation_1.lineAndCharacter(statement.getStart(), file).character;
            if (currentLine > startOfPile.line) {
                if (currentIndentation === startOfPile.character) {
                    lastLineOfPile = navigation_1.lineAndCharacter(statement.getEnd(), file).line;
                }
                else {
                    break;
                }
...
class ChainedStatements {
    constructor(topStatement, prev, next) {
        ...
        this.positions = {
            prevTopStart: navigation_1.lineAndCharacter(this.topStatement.getStart(), file),
            prevTopEnd: navigation_1.lineAndCharacter(this.topStatement.getEnd(), file),
            prevStart: navigation_1.lineAndCharacter(this.prev.getStart(), file),
            prevEnd: navigation_1.lineAndCharacter(this.prev.getEnd(), file),
            nextStart: navigation_1.lineAndCharacter(this.next.getStart(), file),
            nextEnd: navigation_1.lineAndCharacter(this.next.getEnd(), file),
        };
    }
    areUnenclosed() {
