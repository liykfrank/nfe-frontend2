...
class Rule extends tslint.Rules.AbstractRule {
    ...
    static issueNode(functionNode) {
        if (nodes_1.is(functionNode, ts.SyntaxKind.FunctionExpression, ts.SyntaxKind.FunctionDeclaration)) {
            return navigation_1.findChild(functionNode, ts.SyntaxKind.FunctionKeyword);
        }
        if (nodes_1.is(functionNode, ts.SyntaxKind.MethodDeclaration, ts.SyntaxKind.GetAccessor, ts.SyntaxKind.SetAccessor)) {
            return navigation_1.findChild(functionNode, ts.SyntaxKind.Identifier);
        }
        if (nodes_1.is(functionNode, ts.SyntaxKind.Constructor)) {
            return navigation_1.findChild(functionNode, ts.SyntaxKind.ConstructorKeyword);
        }
        if (nodes_1.isArrowFunction(functionNode)) {
