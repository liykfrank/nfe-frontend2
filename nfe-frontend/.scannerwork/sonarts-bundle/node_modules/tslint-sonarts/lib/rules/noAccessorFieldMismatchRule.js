...
class Visitor extends sonarAnalysis_1.SonarRuleVisitor {
    ...
    visitAccessor(accessor, setterOrGetter) {
        ...
        if (containingStructure.kind === ts.SyntaxKind.ObjectLiteralExpression) {
            ...
            matchingField = Visitor.matchingField(Array.from(containingStructure.members), Visitor.fieldsDeclaredInConstructorParameters(containingStructure), setterOrGetter.name);
        ...
        if (accessorIsPublic &&
            ...
            this.addIssue(accessor.name, `Refactor this ${setterOrGetter.type} so that it actually refers to the property '${matchingField.name.getText()}'`);
        }
    }
