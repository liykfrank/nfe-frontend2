...
public class FilesUtilityTest {
    ...
    public void testSaveMultipartFilesFromLocalToRemote() {
        ...
        try {
            filesUtility.saveMultipartFilesFromLocalToRemote(getRefund(), list);
        } catch (Exception e) {
    ...
    public void testGetRemotePathForFile() {
        assertEquals("attached_files/ES/refund/" + getRefund().getId(),
                filesUtility.getRemotePathForFile(getRefund()));
    }

    @Test
    public void testGetRemoteFullPathForFile() {
        assertEquals("attached_files/ES/refund/" + getRefund().getId() + "/file_one.txt",
                filesUtility.getRemoteFullPathForFile(getRefund(), getFiles().get(0).getName()));
    }
    ...
    private Refund getRefund() {
        ...
        refund.setId(789456123L);
        refund.setIsoCountryCode("ES");
        return refund;
    }
    ...
    private List<BsplinkFile> getFiles() {
        ...
        fileOne.setRefundId(getRefund().getId());
        ...
        fileTwo.setRefundId(getRefund().getId());
        ...
        fileThree.setRefundId(getRefund().getId());
