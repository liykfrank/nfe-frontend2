...
import { CardIf } from './../../core/models/card.model';
...
describe('DashboardComponent', () => {
  ...
  beforeEach(async(() => {
    TestBed.configureTestingModule({
      imports: [],
      declarations: [jqxDockingComponent, DashboardComponent],
  ...
  it('should charge cardsCoords in ngAfterViewInit', async(() => {
    const cardsCords = {
      test1: { panel: 0, position: 0 },
      test2: { panel: 1, position: 0 }
    };
  ...
  it('should charge windowOptions in widgetObject', async(() => {
    ...
    const expectedWindowOptions = {
      test1: {
        mode: 'docked',
        resizable: true,
        size: windowOptions.test1.size
      },
      test2: {
        mode: 'docked',
        resizable: true,
        size: windowOptions.test2.size
      }
    };
  ...
  it('should update positions on dragging', async(() => {
    ...
    const cardsCords = {
      test1: { panel: 1, position: 0 },
      test2: { panel: 0, position: 0 }
    };

    component.myDocking.widgetObject = {
      _draggingItem: [{ nextSibling: { id: 'test2' } }]
    };
  ...
  it('should call relocateCardsInWiew method 2 times on dragging', async(() => {
    ...
    component.myDocking.widgetObject = {
      _draggingItem: [{ nextSibling: { id: 'test1' } }]
    };
    component.dragEnd(event);
