...
export class BasicInfoService {
  ...
  constructor(
   ...
   /*  this._CurrencyService.setBaseURL(
 ...
 //  this._PeriodService.getPeriod().subscribe(data => this.$period.next(data));
  ...
  public checkBasicInfo(conf?: AdmAcmConfiguration): string[] {
    ...
    if (
      conf.airlineVatNumberEnabled &&
      (!bi.airlineVatNumber || bi.airlineVatNumber.length == 0)
    ) {
    ...
    if (
      conf.companyRegistrationNumberEnabled &&
      (!bi.airlineRegistrationNumber ||
        bi.airlineRegistrationNumber.length == 0)
    ) {
...
/*
    if (
      conf.agentVatNumberEnabled &&
      (!bi.agentVatNumber || bi.agentVatNumber.length == 0)
    ) {
    ...
    if (
      conf.companyRegistrationNumberEnabled &&
      (!bi.agentRegistrationNumber || bi.agentRegistrationNumber.length == 0)
    ) {
  ...
  public copyToAdcm(acdm: Acdm): void {
    ...
    acdm.airlineRegistrationNumber =
      tempBasicInfoModel.airlineRegistrationNumber;
